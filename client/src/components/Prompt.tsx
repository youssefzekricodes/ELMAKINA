...
/** One line of the result sheet: rank, face, name, trophies. */
function ResultRow({ row, me, pl, scored }: { row: Place; me: string | null; pl: (id?: string | null) => any; scored: boolean }) {
  const p = pl(row.id);
  if (!p) return null;
  return (
...
export function Prompt() {
  ...
  if (st.phase === 'ended') {
    ...
    const champ = places[0] && pl(places[0].id);
    const scored = !!st.standings;
    body = (
      <div className="winner">
        {/* A result sheet, not a trophy ceremony. The winner is the headline — one face, one name,
            one number — and everyone else is a ranked line under a rule. Gold, silver and bronze
            survive as a tint on the rank digits; three gradient plinths were decoration standing in
            for a hierarchy the numbers already state. */}
        ...
        {places.length > 1 && (
          <ul className="res-sheet">
            {places.slice(1).map((row) => <ResultRow key={row.id} row={row} me={me} pl={pl} scored={scored} />)}
          </ul>
        )}
    ...
    if (w.playerId === me && w.data) {
      ...
      if (w.kind === 'lose_card') {
        ...
        body = <LoseCards key={w.deadline} count={w.data.count || 1} canPay={!!w.data.canPay} payCost={w.data.payCost} />;
...
/** The only lose_card prompt left: buy your way out, or take the hit. WHICH card goes is random —
    the player never picks, so there is no card grid here any more. */
function LoseCards({ count, canPay, payCost }: { count: number; canPay: boolean; payCost: number }) {
  return (
    <>
      <div className="p-sub">{count > 1 ? t('decision.chooseN', { n: count }) : t('decision.choose')}</div>
      <div className="p-actions">
        {canPay && <KeyBtn variant="primary" icon="wallet-money" onPress={() => decide({ pay: true })} main={t('decision.pay', { n: payCost })} />}
        <KeyBtn variant="danger" icon="danger-triangle" onPress={() => decide({})} main={t('decision.takeHit')} />
      </div>
    </>
  );
}
