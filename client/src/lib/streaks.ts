...
export interface Streak { count: number; best: number; freezes: number; today: boolean; atRisk: boolean }
...
function apply(count: number, best: number, freezes: number, today: boolean, atRisk = false) {
  store.set({ streak: { count, best, freezes, today, atRisk } });
  updateStreakWidget(count, today);
}
...
export async function initStreaks() {
  ...
  try {
    ...
    if (!data) return;
    apply(data.count || 0, data.best || 0, data.freezes || 0, !!data.today, !!data.at_risk);
...
export async function tickStreak() {
  ...
  try {
    ...
    if (!data) return;
    apply(data.count, data.best, data.freezes, true);
...
export async function saveStreak(): Promise<boolean> {
  ...
  try {
    ...
    if (data) {
      ...
      if (st) store.set({ streak: { ...st, freezes: 1, atRisk: false } });
      track('streak_saved', {});
    }
    return !!data;
  } catch { return false; }
